...
def read_input(path: str) -> pd.DataFrame:
	...
	cols = ["cover_no", "cover_date_jalali", "unit_or_code", "supplier_name", "status", "description"]
	for c in cols:
...
def insert_staging(df: pd.DataFrame, source_filename: str) -> int:
	...
	with conn, conn.cursor() as cur:
		sql = """
			INSERT INTO landing.covers_stage (
				batch_id, source_filename, ingested_at,
				cover_no, cover_date_jalali, unit_or_code, supplier_name, status, description
			) VALUES %s
		"""
...
def main():
	if len(sys.argv) < 2:
		print("Usage: python scripts/load_covers.py <path-to-excel-or-csv>")
		sys.exit(1)

	path = sys.argv[1]
	df = read_input(path)
	count = insert_staging(df, os.path.basename(path))
	print(f"Inserted {count} rows into landing.covers_stage.")
